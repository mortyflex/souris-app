...
const mockPickProductPhotoFromLibrary = jest.fn(() => Promise.resolve(mockPhotoResult));
const mockRemoveImageBackground = jest.fn((uri: string) => Promise.resolve(uri));
  ...
  return (
    <>
      ...
      <Text testID="masque-active">
        {products.find((product) => product.id === '6974bff937a5d89c2d9afbd0')
          ? String(products.find((product) => product.id === '6974bff937a5d89c2d9afbd0')?.active)
          : 'gone'}
      </Text>
...
function SeededImageEditor() {
  ...
  if (open) {
    return (
      <ProductEditorScreen mode="existing" productId="6974bff937a5d89c2d9afbd0" />
    );
  }
...
describe('ProductEditorScreen', () => {
  beforeEach(() => {
    ...
    mockRemoveImageBackground.mockImplementation((uri: string) => Promise.resolve(uri));
  });
  ...
  it('creates a Product from the professional-facing fields', async () => {
    ...
    await act(async () => {
      fireEvent.changeText(view.getByLabelText('Nom du produit'), 'Shampooing Test');
    });
    await act(async () => {
      fireEvent.changeText(view.getByLabelText('Marque du produit'), 'Brand Test');
    });
    await act(async () => {
      fireEvent.changeText(view.getByLabelText('Catégorie du produit'), 'Soin');
    });
    await act(async () => {
      fireEvent.changeText(view.getByLabelText('Code-barres du produit'), '0123456789');
    });
    await act(async () => {
    ...
    expect(view.queryByText('Prenez une photo ou choisissez une image.')).toBeNull();
    expect(view.queryByTestId('product-photo-sheet')).toBeNull();

    await act(async () => fireEvent.press(view.getByLabelText('Ajouter une photo')));
    ...
    await act(async () => fireEvent.press(view.getByTestId('mock-camera-capture')));
    expect(view.queryByTestId('mock-camera-capture')).toBeNull();
    expect(view.queryByTestId('product-photo-sheet')).toBeNull();
    expect(mockPickProductPhotoFromLibrary).not.toHaveBeenCalled();
    expect(mockRemoveImageBackground).toHaveBeenCalledWith('file:///products/raw.jpg');
    expect(view.getByTestId('product-form-image-source').props.source).toEqual({
      uri: REPLACEMENT_IMAGE_URI,
    });
    expect(view.getByTestId('product-form-image-source').props.contentFit).toBe('contain');
    ...
    expect(view.getByLabelText('Nom du produit').props.value).toBe('Shampooing Test');
    expect(view.getByLabelText('Code-barres du produit').props.value).toBe('0123456789');
    ...
    expect(stored).toContain(':Shampooing Test:Brand Test:Soin:0123456789:25:4:true');
    expect(view.getByTestId('managed-image').props.children).toBe(REPLACEMENT_IMAGE_URI);
  ...
  it('rejects invalid stock and price inputs', async () => {
    ...
    expect(view.getByTestId('save-product').props.accessibilityState?.disabled).toBe(true);
  });
  ...
  it('keeps an image replacement in the edit draft until Save and discards it on Cancel', async () => {
    ...
    await act(async () => fireEvent.press(view.getByTestId('open-seeded-image-product')));
    expect(view.getByTestId('product-detail-image-source').props.source).toEqual({
      uri: ORIGINAL_IMAGE_URI,
    });
    expect(view.getByTestId('product-detail-image-source').props.contentFit).toBe('contain');
    ...
    await act(async () => fireEvent.press(view.getByLabelText('Modifier la photo')));
    ...
    expect(view.getByTestId('managed-image').props.children).toBe(ORIGINAL_IMAGE_URI);
  ...
  it('can save the selected original while optional image processing is still pending', async () => {
    ...
    expect(view.getByLabelText('Modifier la photo').props.accessibilityState).toEqual({
      busy: true,
      disabled: true,
    });
    expect(view.getByTestId('save-product').props.accessibilityState?.disabled).toBe(false);

    await act(async () => fireEvent.press(view.getByTestId('save-product')));
    expect(view.getByTestId('managed-image').props.children).toBe('file:///products/raw.jpg');
  ...
  it('keeps the Product form usable when photo permission is denied', async () => {
    ...
    await act(async () => fireEvent.changeText(view.getByLabelText('Nom du produit'), 'Soin Test'));
    await act(async () => fireEvent.press(view.getByLabelText('Ajouter une photo')));
    await act(async () => fireEvent.press(view.getByLabelText('Choisir dans la photothèque')));
  ...
  it('deletes only after explicit confirmation and closes the details', async () => {
    ...
    await act(async () => {
      fireEvent.press(view.getByLabelText('Supprimer définitivement ce produit'));
    });
    ...
    expect(view.getByTestId('managed-product').props.children).toContain('Masque réparateur');
  ...
  it('prefills the create draft with the exact route barcode and allows duplicates', async () => {
    ...
    expect(view.getByLabelText('Code-barres du produit').props.value).toBe('3474637152000');
    expect(view.getByText('Ce code-barres est aussi utilisé par « Masque réparateur 5 min ».')).toBeTruthy();
  ...
  it('keeps a scanned create barcode in the draft until normal save', async () => {
    ...
    await act(async () => fireEvent.press(view.getByLabelText('Scanner le code-barres')));
    await act(async () => fireEvent.press(view.getByTestId('mock-camera-detection')));

    expect(view.getByLabelText('Code-barres du produit').props.value).toBe('0012345678901');
    expect(view.getByTestId('catalog-count').props.children).toBe(50);

    await act(async () => fireEvent.changeText(view.getByLabelText('Nom du produit'), 'Shampooing Test'));
    await act(async () => fireEvent.changeText(view.getByLabelText('Prix du produit'), '25'));
    await act(async () => fireEvent.press(view.getByTestId('save-product')));
  ...
  it('discards or saves a scanned edit barcode through the existing form actions', async () => {
    ...
    expect(view.getByLabelText('Code-barres du produit').props.value).toBe('0012345678901');
    expect(view.getByTestId('managed-product').props.children).toContain(':3474637152000:');
    ...
    expect(view.getByTestId('managed-product').props.children).toContain(':0012345678901:');
  });
});
